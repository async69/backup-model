  ...
  return (
    <Fragment>
      <View style={styles.headerRight}>
        ...
        <Text>Purchse order No.: {invoice.invoice_no} </Text>
        <Text>Posting Date: {invoice.trans_date} </Text>

      </View>

      <View style={styles.headerRight}>
        <Text>Purchase Type: Purchse Typeee</Text>
        <Text>Order Date : {invoice.trans_date}</Text>
      </View>

      <View style={styles.headerLeft}>
      </View>
      <Text>Vendor Shipment N0. : {invoice.invoice_no} </Text>
      <Text>Remark  : {invoice.invoice_no} </Text>
      {/* <View style={styles.headerLeft}>
...
const Print = ({ invoice }) => (
  <Document>
    <Page orientation="landscape" size="A4" style={styles.page}>
      <Header />
      <Text style={styles.reportTitle}> Goods Receving Note </Text>
